...
class plot :
    def __init__(self):
        self.sub1 = rospy.Subscriber('path2', Path, self.call1)
    ...
    def call1(self,msg):
        ...
        while i<5:
            ...
            if (distance >= 0.05):
                ...
                if(abs(theta2 - self.ang) > 0.1):
                    self.vel.linear.x =0
                    self.vel.angular.z = 0.05 * ((theta2 - self.ang))
                    self.pub.publish(self.vel)
                    

                else:
                    self.vel.linear.x = 0.05 * distance
